...
Like Iris: compute shaders (the light grid and the world GI cache) are switched to
the core profile, and the MC_RENDER_STAGE_* defines that Iris injects are added.
The number of images each program uses is checked against NVIDIA's limit (8), and
the number of textures (samplers) against the 16 that OpenGL guarantees every
program (Iris stops loading the pack when a program needs more units than the
driver has; most drivers have 32, but 16 is all that is promised).
"""
...
MAX_TEXTURE_UNITS_GUARANTEED = 16
...
GL_VERTEX_SHADER = 0x8B31; GL_FRAGMENT_SHADER = 0x8B30; GL_COMPUTE_SHADER = 0x91B9; GL_GEOMETRY_SHADER = 0x8DD9
...
# sampler types (2D/3D, shadow, integer and unsigned variants) as reported by glGetActiveUniform
SAMPLER_TYPES = {0x8B5E, 0x8B5F, 0x8B60, 0x8B62, 0x8DC9, 0x8DCA, 0x8DCB, 0x8DCF, 0x8DD2, 0x8DD3,
                 0x8DD4, 0x8DD5, 0x8DD7}

def sampler_uniforms(p):
    ...
    for i in range(cnt.value):
        ...
        if ty.value in SAMPLER_TYPES: out.append(nm.value.decode())
    return out
...
def main():
    ...
    for w in sorted(d for d in os.listdir(ROOT) if d.startswith("world")):
        ...
        for base in sorted({os.path.splitext(f)[0] for f in files if f.endswith((".vsh", ".gsh", ".fsh", ".csh"))}):
            if only and base not in only: continue
            if base + ".csh" in files:
                stages = [(GL_COMPUTE_SHADER, base + ".csh", True)]
            else:
                stages = [(k, base + e, False) for k, e in ((GL_VERTEX_SHADER, ".vsh"), (GL_GEOMETRY_SHADER, ".gsh"),
            ...
            if not logs:
                ...
                if not ok:
                    ...
                    smps = sampler_uniforms(p)
                    if len(imgs) > MAX_IMAGE_UNITS_NVIDIA:
                        logs.append(f"uses {len(imgs)} images, NVIDIA allows at most {MAX_IMAGE_UNITS_NVIDIA}: {imgs}")
                    if len(smps) > MAX_TEXTURE_UNITS_GUARANTEED:
                        logs.append(f"uses {len(smps)} textures, OpenGL guarantees only {MAX_TEXTURE_UNITS_GUARANTEED}: {smps}")
                    if only and not logs:
                        print(f"  {w}/{base}: {len(imgs)} image {imgs} | {len(smps)} textures")
            if logs:
                fails += 1
