...
code_text = "\n".join(code_text)
#     An option that only steers a preprocessor condition inside settings.glsl is read
#     too (SHADOW_DISTANCE becomes Iris' shadowDistance there, CUTOUT_MASK_RES the mask
#     resolution in use, REFLECTION_MODE the two reflection switches).
...
IRIS_BUILTIN = {"shadowMapResolution", "sunPathRotation"}
for line in joined.split("\n"):
    st = line.strip()
    ...
    for dp, _, fns in os.walk(os.path.join(PROJECT, sub)):
        for fn in sorted(fns):
            ...
            for no, line in enumerate(open(path, encoding="utf-8").read().split("\n"), 1):
                ...
                # Iris reads everything after '#include' as the file name: a comment behind
                # the path becomes part of it and the pack fails to load ("file not found")
                if fn.endswith(SHADER_EXT) and line.strip().startswith("#include") \
                        and not re.fullmatch(r'\s*#include\s+"[^"]+"\s*', line):
                    errors.append(f"{rel}:{no}: #include must be followed by the quoted path only: {line.strip()[:70]}")
