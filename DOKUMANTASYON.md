...
2.0 (çalışan taban) üzerine üç özellik eklendi. Her biri ayrı ayrı ve birlikte
derleme testinden geçirildi, `shaders.properties` başlığının bozulmadığı
bayt karşılaştırmasıyla doğrulandı.

- **TAA** (`TAA`, varsayılan açık): kamera her karede Halton dizisiyle bir
  pikselden az kaydırılır (`lib/taa.glsl`), sonuçlar `taa_resolve` geçişinde
  yeniden projeksiyonla birleştirilir. Geçmiş, bu karenin 3x3 komşuluğunun renk
  aralığına kırpılır; hayalet izini engelleyen adım budur. Yeni tampon:
  `colortex10` (kareler arası korunur). Gölge geçişine titreşim UYGULANMAZ,
  yoksa voxelleştirme ve gölge kenarları etkilenirdi.
- **Normal haritası** (`MATERIAL_NORMALS`, varsayılan kapalı): LabPBR `_n`
  dokusunun rg kanalı okunur, teğet uzayı `at_tangent`'ten kurulur.
- **Paralaks / POM** (`MATERIAL_POM`, varsayılan kapalı): yükseklik haritasında
  teğet uzayında yürünür; doku koordinatı `wrapInSprite()` ile kendi sprite'ı
  içinde tutulur, aksi halde atlasta komşu bloğun dokusu okunurdu.
- Normal haritası ve POM LabPBR kaynak paketi gerektirdiği için varsayılan
  kapalıdır; kaynak pakette veri yoksa matematik geometrik normale geri döner.


### 2.0

Bu sürüm, GI'ın çalıştığı doğrulanan **1.2 dosyalarından** yeniden kuruldu.
1.3 ile 1.11 arasındaki hiçbir değişiklik taşınmadı; üzerine yalnızca SVGF
gürültü giderici eklendi.

- **SVGF tarzı gürültü giderici** (`DENOISER_ATROUS`, varsayılan açık):
  `denoise_seed` varyansı üretir, `denoise1..3` à-trous turlarını çalıştırır.
  Ayrıntılar `lib/atrous.glsl` başında.
- Geçiş sırası: `deferred2..5` gürültü giderme, `deferred6` aydınlatma.
  Iris geçişleri numara sırasıyla çalıştırdığı için gürültü giderme
  aydınlatmadan önce bitmek zorundadır.
- Yeni tamponlar: `colortex8`, `colortex9` (à-trous ping-pong).
- Yeni araç `tools/audit.py`: uniform adları, örnekleyiciler, render hedefleri,
  `image.*` yönergeleri ve geçiş sırası tutarlılığını oyunu açmadan denetler.

**1.3-1.11'de neyin bozulduğu:** Kesin sebep bulunamadı. Karşılaştırmada
`shaders.properties` başlık yorumunun bir düzenleme betiği tarafından
bozulduğu görüldü (yorum satırına satır devam karakterleri girmişti); bu
sürümde o dosya çalışan halinden alındığı için sorun ortadan kalktı.
Aradaki diğer eklentiler (TAA, POM, hacimsel bulutlar, uyumluluk modu,
GLSL 3.3'e düşürme) bilinçli olarak dışarıda bırakıldı.
